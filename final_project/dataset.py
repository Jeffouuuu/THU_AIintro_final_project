...
class AudioVideoDataset(FairseqDataset):
    ...
    def get_label(self, index):
        ...
        with open(self.label_path, mode = 'rb') as f:
            f.seek(s)
            tokens_string = f.read(e-s).decode('utf-8')
        tokens_line = self.bpe_tokenizer().encode(tokens_string)
        label = self.dictionary().encode_line(tokens_line,add_if_not_exist=False)
    ...
    def load_video(self, video_name):
        ...
        if not video.isOpend():
            logger.info(f"Unable to read {video_name}")
        frames = []
        while True:
            read, frame = video.read()
            if not read : #the latest frame was the last or error while reading frame
                break
        ...
        T = len(frames)
        H ,W ,C = frames[0].shape # C = 1
        feats = np.reshape(frames,(T,H,W,C))
