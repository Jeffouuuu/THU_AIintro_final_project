...
class AudioVideoDataset(FairseqDataset):
    ...
    def load_audio(self, audio_name):
        ...
        data_cupy = cupy.array(data)
        eng, tot_eng = logfbank(data_cupy)
        num_frames = eng.shape[0]
        padding = -num_frames%self.stack_order_audio
        if padding > 0:
            eng = cupy.pad(eng,(0,padding),(0,0),'constant')
        audio_feats = cupy.reshape(eng, (-1, self.stack_order_audio * eng.shape[1]))
